...
from frappe.utils import data, getdate, nowdate
...
class PartyLedgerSummaryReport(object):
	...
	def run(self, args):
		...
		data = self.get_data()
		res2 = [a_dict['party'] for a_dict in data]

		dd = frappe.db.sql(f"""  select 
									customer,
									sum(rounded_total) as amount,  
									CONCAT(MONTHNAME(posting_date)," " ,YEAR(posting_date)) as date,
									DATE_SUB(posting_date,INTERVAL DAYOFMONTH(posting_date)-1 DAY) AS 'first',
									LAST_DAY(posting_date) as 'last'
								from `tabSales Invoice`   
								where docstatus = 1 and 
								posting_date between "{self.filters.from_date}" and "{self.filters.to_date}"  
								GROUP BY customer, YEAR(posting_date), MONTH(posting_date)   
								""",as_dict=1)
		for i in dd:
			# out[i.customer]
			index = res2.index(i.customer)
			ff = data[index]
			ff[i.date]=i.amount

		dd2 = frappe.db.sql(f"""  select 
									CONCAT(MONTHNAME(posting_date)," " ,YEAR(posting_date)) as date
									from `tabSales Invoice`   
								where docstatus = 1 and 
								posting_date between "{self.filters.from_date}" and "{self.filters.to_date}"  
