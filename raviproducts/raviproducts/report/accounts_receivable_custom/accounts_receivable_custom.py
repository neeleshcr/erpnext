...
class ReceivablePayableReport(object):
	...
	def append_row(self, row):
		...
		self.set_ageing(row)
		self.get_last_invoice(row,row.party)
		...
		# row.update({'java':str(self.get_last_invoice(row,row.party))})
		self.data.append(row)
		# self.data.append({'java':'java'})
	def get_last_invoice(self, row,name):
		dd = frappe.db.sql(f""" select
				   DATE(creation) AS ID ,
				   name ,
				   paid_amount
				   from `tabPayment Entry`
				   WHERE  DATE(creation)  <= "{self.filters.report_date}" and party =  "{name}"
				   order by  DATE(creation)  desc  limit 4 
				   """,
				   as_dict=1)
		for i in range(len(dd)):
			value = dd[i]
			row[f'date{i+1}']=value.ID
			row[f'name{i+1}']=value.paid_amount
		# return ss
	...
	# def get_cusomer_last_invoice(self):
