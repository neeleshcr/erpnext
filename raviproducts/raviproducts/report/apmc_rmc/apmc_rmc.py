# Copyright (c) 2024, VPS Consultancy and contributors
...
def execute(filters=None):
	...
	for sle in sl_entries:
		...
		sle["date"] = sle["date"].strftime("%d-%m-%Y")

		if sle["voucher_type"] == "Purchase Receipt":

			sle["stock_entry_type"] = "Purchase/Return"
			pr_doc = frappe.get_doc("Purchase Receipt", sle["voucher_no"])

			sle["supplier_invoice_no"] = pr_doc.supplier_invoice_no
			sle["supplier_name"] = pr_doc.supplier
			sle["amount"] = (frappe.get_value("Purchase Receipt Item", sle["voucher_detail_no"], "amount") or 0) + (frappe.get_value("Purchase Receipt Item", sle["voucher_detail_no"], "cgst_amount") or 0) + (frappe.get_value("Purchase Receipt Item", sle["voucher_detail_no"], "sgst_amount") or 0) +(frappe.get_value("Purchase Receipt Item", sle["voucher_detail_no"], "igst_amount") or 0)
			sle["purchased_from"] = frappe.get_value("Address", pr_doc.supplier_address, "state")
			
			if frappe.get_value("Address", pr_doc.supplier_address, "gst_state") == frappe.get_value("Address", pr_doc.billing_address, "gst_state"):
				sle["local_purchase_qty"] = sle["in_qty"]
				sle["interstate_purchase_qty"] = 0
			
			else:
				sle["interstate_purchase_qty"] = sle["in_qty"]
				sle["local_purchase_qty"] = 0
		
		if sle["voucher_type"] == "Stock Entry":
			sle["stock_entry_type"] = frappe.get_value("Stock Entry", sle["voucher_no"], "stock_entry_type")
...
def get_columns(filters):
	columns = [
		...
		{
				"label": _("Purchased From"),
				"fieldname": "purchased_from",
				"fieldtype": "Data",
				"width": 150,
		},
		{
			"label": _("Local Purchase Qty"),
			"fieldname": "local_purchase_qty",
			"fieldtype": "Float",
			"width": 150,
			"convertible": "qty",
		},
		{
			"label": _("Interstate Purchase Qty"),
			"fieldname": "interstate_purchase_qty",
			"fieldtype": "Float",
			"width": 150,
			"convertible": "qty",
		},
